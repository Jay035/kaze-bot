...
export default function Navbar({}: Props) {
  ...
  return (
    <nav
      data-aos="fade-in"
      className="absolute top-0 left-0 z-30 flex justify-between items-center tracking-tight w-full gap-x-12 py-4 lg:py-8 pr-5 lg:pr-10 xl:px-28"
    >
      <section className="pl-5 lg:pl-10 xl:pl-0">
      ...
      >
        <Link
          ...
          className="flex items-center justify-center w-full py-4 border-y border-white/25 lg:border-none hover:text-[#69FF77] hover:font-bold text-lg sm:text-xl lg:text-2xl tracking-[-0.03rem] text-center text-white "
        >
          Tools
        </Link>
        <Link
        ...
        <Link
          href="/wallet"
          onClick={() => setMenuOpen((prevState) => !prevState)}
          className="bg-white lg:hidden text-center mt-10 text-lg w-48 hover:border hover:border-white hover:bg-transparent hover:text-white sm:text-xl lg:text-2xl text-[#131313] rounded-[6.25rem] p-[0.6rem]"
        >
          {wallet && recentWallet
            ? "Connect wallet"
      ...
      <Link
        ...
        className="bg-white hidden text-center lg:block text-lg w-48 hover:border hover:border-white hover:bg-transparent hover:text-white sm:text-xl lg:text-2xl text-[#131313] rounded-[6.25rem] p-[0.6rem]"
      >
        {!wallet && !recentWallet
          ? "Connect wallet"
          : recentWallet?.account.address.toString()
